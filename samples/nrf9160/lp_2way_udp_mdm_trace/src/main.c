...
void socket_poll_test(int infd)
{
    struct pollfd fd;
    int ret;
    int cnt = 0;
...
workerror:
	...
	err = send(client_fd_RAI, buffer, 2, 0);
	if (err < 0) {
		printk("Failed to transmit UDP packet, %d\n", errno);
		return;
	}
...
static int server_connect(void)
{
    ...
    client_fd_RAI = socket(AF_INET,  SOCK_DGRAM , IPPROTO_UDP);;
#else
	...
	err = connect(client_fd_RAI, (struct sockaddr *)&host_addr,
		      sizeof(struct sockaddr_in));
	if (err < 0) {
		printk("Connect failed : %d\n", errno);
		goto error;
