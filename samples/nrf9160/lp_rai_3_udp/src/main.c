...
static void server_transmission_work_fn(struct k_work *work)
{
    ...
    char recv_buf[CONFIG_UDP_DATA_UPLOAD_SIZE_BYTES * 3]; /* larger recv buffer to be sure all data received */
    ...
    buffer[strlen(buffer)] = '#'; /* replace 0 with '#' */
#else
	...
	err = recv(client_fd, recv_buf, CONFIG_UDP_DATA_UPLOAD_SIZE_BYTES, MSG_WAITALL);
	if (err < 0) {
		printk("Failed to receive UDP packet, %d\n", errno);
        goto workerror;
		/* return; */ /* do not return */
	}  
    recv_buf[sizeof(recv_buf)-1] = 0; /* set end of the string for debugging output  */
    printk("recv: %s\n", recv_buf);
workerror:
#endif
#if defined(CONFIG_RAI_ENABLE_AFTER_PACKET_SEND)
