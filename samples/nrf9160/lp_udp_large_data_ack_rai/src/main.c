...
int socket_option_rel14_req(bool enable)
{
	...
	if (enable) {
		err = nrf_modem_at_printf("AT%%REL14FEAT=1,1,0,0,1");
	} else {
		err = nrf_modem_at_printf("AT%%REL14FEAT=1,0,0,0,1");
	}
...
int deep_search_config(bool enable)
{
	...
	if (enable) {
		err = nrf_modem_at_printf("AT%%XDEEPSEARCH=1");
	} else {
		err = nrf_modem_at_printf("AT%%XDEEPSEARCH=0");
	}
...
static void server_transmission_work_fn(struct k_work *work)
{
	...
	while(cnt < CONFIG_UDP_TOTAL_DATA_KB){  /* moddify here for total packets n x K byes.  */
		...
		SO_RAI_WAIT_MORE:
		*/
		int option = SO_RAI_ONGOING;  /* 3way, now on going, which expecting a packet return from server */
		err = setsockopt(client_fd, SOL_SOCKET, option, NULL, 0);
		if (err < 0) {
			printk("setsockopt(%d) error: %d", option, err);
			goto workerror;
		...
		if (err < 0) {
			...
			memset(buffer, '#', sizeof(buffer));
			buffer[sizeof(buffer) - 1] = 0; /* set string end */
			sprintf(buffer, "ack: %d not received from server", cnt);
			buffer[strlen(buffer)] = ' ';   /* remove the 0 at endf of pkge: xx  */
			...
			SO_RAI_WAIT_MORE:
			*/
			/* socket_option_rai_req(true); */
			option = SO_RAI_LAST;  /* 3way, now on going, which expecting a packet return from server */
			err = setsockopt(client_fd, SOL_SOCKET, option, NULL, 0);
			if (err < 0) {
				printk("setsockopt(%d) error: %d", option, err);
				goto workerror;
			...
			err = send(client_fd, buffer, 10, 0);
			if (err < 0) {
				printk("Failed to transmit UDP packet, %d\n", errno);
				goto workerror;
				/* return; */ /* do not return */
			}	
			printk("send ACK %d to server, fail receive from server\n", cnt);	
			goto workerror;
			/* return; */ /* do not return */
		}  
		printk("recv: %s\n", buffer);
	...
	memset(buffer, '#', sizeof(buffer));
	buffer[sizeof(buffer) - 1] = 0; /* set string end */
	sprintf(buffer, "ack: %d received from server", cnt);
	buffer[strlen(buffer)] = ' ';   /* remove the 0 at endf of pkge: xx  */
	...
	*/
	/* socket_option_rai_req(true); */
	int option = SO_RAI_LAST;  /* 3way, now on going, which expecting a packet return from server */
	err = setsockopt(client_fd, SOL_SOCKET, option, NULL, 0);
	if (err < 0) {
		printk("setsockopt(%d) error: %d", option, err);
		goto workerror;
	...
	printk("send ACK %d to server, received packet from server\n", cnt);			
...
static int configure_low_power(void)
{
	...
	err = modem_sleep_report_req(false);
	if(err) {
