...
const uint32_t counter_read_pin = 7;
...
static void gpiote_init(void)
{
	...
	err = nrfx_gpiote_in_init(counting_pin, &rising_edge, NULL);
	if (err != NRFX_SUCCESS) {
		printk("gpiote1 err: %x", err);
		return;
	}

	err = nrfx_gpiote_in_init(counter_read_pin, &falling_edge,
	...
	nrfx_gpiote_in_event_enable(counter_read_pin, 1);
	nrfx_gpiote_in_event_enable(counting_pin, 1);
}
